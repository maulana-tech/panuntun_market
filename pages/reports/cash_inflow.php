...
// Get cash inflow data with sales details
$query = "SELECT 
    km.tanggal,
    km.jumlah,
    km.keterangan,
    p.jumlah as qty_sold,
    p.harga_satuan,
    b.nama_barang,
    u.nama as user_name
FROM kas_masuk km
...
WHERE DATE(km.tanggal) BETWEEN :start_date AND :end_date
...
<div class="px-6 py-4 border-b border-gray-200">
    <h3 class="text-lg font-medium text-gray-900">Cash Inflow Report</h3>
...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
            <div class="text-sm font-medium text-green-800">Total Transactions</div>
            <div class="text-xl font-bold text-green-600"><?php echo $summary_stats['total_transactions'] ?? 0; ?></div>
        </div>
        
        <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
            <div class="text-sm font-medium text-green-800">Total Amount</div>
            <div class="text-xl font-bold text-green-600"><?php echo formatCurrency($summary_stats['total_amount'] ?? 0); ?></div>
        </div>
        ...
        <div class="bg-purple-50 border border-purple-200 rounded-lg p-4 text-center">
            <div class="text-sm font-medium text-purple-800">Highest Sale</div>
            <div class="text-xl font-bold text-purple-600"><?php echo formatCurrency($summary_stats['max_amount'] ?? 0); ?></div>
        </div>
        
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
            <div class="text-sm font-medium text-gray-800">Lowest Sale</div>
                    ...
                    <?php if (empty($cash_inflow_data)): ?>
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                No cash inflow transactions found for the selected period.
                            </td>
                        </tr>
                    <?php else: ?>
